...
install_marzban_script() {
    ...
    SCRIPT_URL="https://github.com/$FETCH_REPO/raw/main/marzban/marzban7.sh"
...
get_versions() {
    VERSIONS=($(curl -s "https://api.github.com/repos/Gozargah/Marzban/releases?per_page=6" | grep -oP '"tag_name": "\K(.*?)(?=")'))
}
...
install_command() {
    ...
    if [ "$USE_VERSION_SELECTION" = true ]; then
        marzban_version=$(select_version)
